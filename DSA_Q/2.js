...
function maxScore(s) {
  ...
  // Loop through the string, stopping before the last character
  for (let i = 1; i < s.length; i++) {
    const left = s.slice(0, i);
    const right = s.slice(i);

    // Count zeros in the left substring and ones in the right substring
    const leftZeros = [...left].filter((char) => char === "0").length;
    const rightOnes = [...right].filter((char) => char === "1").length;
